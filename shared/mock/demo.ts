import type { RiderState, WSMessage } from '../types';
...
export function sceneToFramePayload(scene: MockScene): WSMessage['payload'] {
  return {
    frameUrl: scene.image,
    lat: scene.lat,
    lng: scene.lng,
    heading: scene.heading,
    city: scene.city,
    state: scene.state,
    roadName: scene.roadName,
    miles: scene.miles,
    milesRemaining: scene.milesRemaining,
    isMilestone: !!scene.isMilestone,
    milestoneName: scene.milestoneName ?? null,
...
export function sceneToRiderState(scene: MockScene, tripId = 'mock-trip', index = 0): RiderState {
  return {
    id: 1,
    trip_id: tripId,
    current_waypoint_index: index,
    current_lat: scene.lat,
    current_lng: scene.lng,
    current_heading: scene.heading,
    current_city: scene.city,
    current_state: scene.state,
    miles_traveled: scene.miles,
    miles_remaining: scene.milesRemaining,
    started_at: new Date().toISOString(),
    estimated_arrival: null,
    is_paused: false,
    pause_reason: null,
    last_frame_at: new Date().toISOString(),
    last_frame_url: scene.image,
    speed_multiplier: 1,
  };
}
